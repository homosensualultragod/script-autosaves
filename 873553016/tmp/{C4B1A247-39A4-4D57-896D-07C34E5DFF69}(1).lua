...
until Character
	...
	framesk4 = makeframe(scrn, 1, UDim2.new(.355, 0, 0.795, 0), UDim2.new(0.605, 0, 0.1, 0), skillcolorscheme)
	framesk5 = makeframe(scrn, 1, UDim2.new(.05, 0, 0.9, 0), UDim2.new(0.91, 0, 0.1, 0), skillcolorscheme)
	...
	bar4 = makeframe(framesk4, 1, UDim2.new(0, 0, 0, 0), UDim2.new(1, 0, 1, 0), skillcolorscheme)
	bar5 = makeframe(framesk5, 1, UDim2.new(0, 0, 0, 0), UDim2.new(1, 0, 1, 0), skillcolorscheme)
...
ParticleEmitter = {Create = function(Parent, Color1, Color2, LightEmission, Size, Texture, Transparency, ZOffset, Accel, Drag, LockedToPart, VelocityInheritance, EmissionDirection, Enabled, LifeTime, Rate, Rotation, RotSpeed, Speed, VelocitySpread)
	...
	Mag = CreatePart(m, Enum.Material.Plastic, 0, 0, "Black", "Hitbox", Vector3.new(.3, 1.2, 0.3))
	MagWeld = CreateWeld(m, Guhn, Mag, CFrame.new(0, 0, 0, 1, 0, 0, 0, 1, 0, 0, 0, 1), CFrame.new(-1, .4, 0, 1, 0, 0, 0, 1, 0, 0, 0, 1))
		...
		if hum ~= nil then
			...
			if head ~= nil then
				...
				if mag <= magni and c.Name ~= Player.Name then
					Damagefunc(head, head, mindam+(magni - mag), maxdam+(magni - mag), knock, Type, RootPart, 0.1, "http://www.roblox.com/asset/?id=160432334", 1)
					-- (mindamage+(magnitude - aoemagnitude), mindamage+(magnitude - aoemagnitude))
				end
			end
		end
	end
...
Break = {Create = function(brickcolor, cframe, x1, y1, z1)
	...
	local speed = 3
	local num = 250
	local dermage = ((-cooldown1 + 155)*0.1)
	local maybe = selectfire
	MouseLook = MouseLook * angles(math.rad(math.random(-SpreadAmount,SpreadAmount)),math.rad(math.random(-SpreadAmount,SpreadAmount)),math.rad(math.random(-SpreadAmount,SpreadAmount)))
	coroutine.resume(coroutine.create(function()
		...
		repeat
			...
			Laser(BrickColor.new(color1), CFrame.new((MainPos + pos) / 2, pos) * angles(1.57, 0, 0), 2, mag * (speed / (speed / 2)), 2, -.5, 0, -.5, 0.05)
			...
			do
				if num <= 0 then
					...
					if hit ~= nil then
						...
						Effects.Block.Create(color1, refpart2.CFrame, 1,1,1,30,30,30,0.05)
						Effects.Block.Create(color2, refpart2.CFrame, 1, 1, 1, 25,2.5,2.5, 0.05)
						MagniDamage(refpart2, 20, 1  * dermage , 1  * dermage, 1, "Normal")
						swait()
						end
...
shoottraildd2 = function(mouse, partt, SpreadAmount)
	...
	coroutine.resume(coroutine.create(function()
		...
		repeat
			swait()
			MainPos2 = mouse --+ SpreadVectors
			MouseLook = CFrame.new((MainPos + MainPos2) / 2, MainPos2)
			local hit, pos = rayCast(MainPos, MouseLook.lookVector, speed, RootPart.Parent)
			local mag = (MainPos - pos).magnitude
			
			BlockEffect(BrickColor.new("Really blue"), CFrame.new((MainPos + pos) / 2, pos) * angles(1.57, 0, 0), 2, 2, 2,  2.5 * universalnum/15, 2.5*universalnum/15, 2.5 * universalnum/15, 0.15, 2)
			MainPos = MainPos + MouseLook.lookVector * speed
			universalnum = universalnum - .05
			MouseLook = MouseLook * angles(math.rad(0), 0, 0)
			if hit ~= nil then
				universalnum = universalnum
				local refpart = CreatePart(EffectModel, "SmoothPlastic", 0, 1, BrickColor.new("Really black"), "Effect", Vector3.new())
				refpart.Anchored = true
				refpart.CFrame = CFrame.new(pos)
				game:GetService("Debris"):AddItem(refpart, 1)
			...
			do
				if universalnum <= 13 then
					universalnum = 0
					local refpart2 = CreatePart(EffectModel, "SmoothPlastic", 0, 1, BrickColor.new("Really black"), "Effect", Vector3.new())
					refpart2.Anchored = true
					refpart2.CFrame = CFrame.new(pos)
					game:GetService("Debris"):AddItem(refpart2, 1)
					shoottraildd3(Player:GetMouse(), refpart2, (15))
					shoottraildd3(Player:GetMouse(), refpart2, (15))
				end
			end
		until universalnum <= 0
	end
))
end
...
shoottraildd3 = function(mouse, partt, SpreadAmount)
	...
	coroutine.resume(coroutine.create(function()
		...
		repeat
			...
			local hit, pos = rayCast(MainPos, MouseLook.lookVector, speed, RootPart.Parent)
			local mag = (MainPos - pos).magnitude
			Laser(BrickColor.new(color1), CFrame.new((MainPos + pos) / 2, pos) * angles(1.57, 0, 0), 2, mag * (speed / (speed / 2)), 2, -.2, 0, -.2, 0.10)
			MainPos = MainPos + MouseLook.lookVector * speed
			num = num - 1
			MouseLook = MouseLook * angles(math.rad(0), 0, 0)
			if hit ~= nil then
				...
				MagniDamage(refpart2, 20, 3 , 3, 1, "Normal")
			end
			do
				if num <= 0 then
					local refpart2 = CreatePart(EffectModel, "SmoothPlastic", 0, 1, BrickColor.new("Really black"), "Effect", Vector3.new())
					...
					game:GetService("Debris"):AddItem(refpart2, 1)
					if hit ~= nil then
						Effects.Block.Create(color1, refpart2.CFrame, 1,1,1,10,10,10,0.35)
						MagniDamage(refpart2, 20, 3 , 3, 1, "Normal")
						swait()						
					end
				end
		...
		while shoot do
			if shoot == true and selectfire == 1 then
				...
				if cooldown1 >= 1 then
					...
					shoottraildd(mouse, Barrel, (overheat/4))
				else
					soe:Stop()
					reload()
					aiming = false
	...
	attackone = function()
		...
		for i = 0,10,1 do
		ddd = ddd + 1
		shoottraildd2(Vector3.new(Torso.CFrame.X, Torso.CFrame.Y + 25, Torso.CFrame.Z), Barrel, (0))
		swait(2)
		end
		attack = false	
	end
	...
	text7.Text = "Damage = " .. 1+((-cooldown1 + 155)*0.1) .. ""
	end
	if selectfire == 2 then
	text7.Text = "Damage = " .. 11+((-cooldown1 + 155)*0.1) .. ""
	end
	if overheat > 0 and shoot == false and cooldown6 <= 0 then
	overheat = overheat - 0.1233333333333333
	end
	...
	if cooldown6 >= 0 then
		...
		if overheat > 0.01 then
		overheat = overheat - 0.01
		end
	end
					...
					if 16565656565656565 < RootPart.Velocity.y and hit == nil then
						...
						if RootPart.Velocity.y < -65656565665561 and hit == nil then
							...
							if Torsovelocity < 1 and hit ~= nil then
								Anim = "Idle"
								if attack == false or reloading == false then
									change = 1
											RootJoint.C0 = clerp(RootJoint.C0, RootCF * cf(0, 0, 0 - 0.15 * math.cos(sine / 20)) * angles(math.rad(90), math.rad(90), math.rad(0)), 0.3)
											Torso.Neck.C0 = clerp(Torso.Neck.C0+Vector3.new(0,0.25,-.1), NeckCF * angles(math.rad(30), math.rad(-90), math.rad(-90)), 0.3)
											RW.C0 = clerp(RW.C0, CFrame.new(1, 0.5, -0.5) * angles(math.rad(90), math.rad(0), math.rad(-90)), 0.3)
											LW.C0 = clerp(LW.C0, CFrame.new(-1.5, 0.5 - 0.1 * math.cos(sine / 25), 0) * angles(math.rad(90), math.rad(0), math.rad(-70)), 0.3)
											RH.C0 = clerp(RH.C0, cf(.5, -2, 0) * RHCF * angles(math.rad(0), math.rad(-40), math.rad(0)), 0.3)
											LH.C0 = clerp(LH.C0, cf(-.5, -2, 0) * LHCF * angles(math.rad(0), math.rad(130), math.rad(0)), 0.3)
											FakeHandleWeld.C0 = clerp(FakeHandleWeld.C0, CFrame.new(0, 0, 0) * angles(math.rad(0), math.rad(0), math.rad(-60)), 0.3)end
							else
								if 1 < Torsovelocity and hit ~= nil then
									Anim = "Walk"
									if attack == false or reloading == false then
										RootJoint.C0 = clerp(RootJoint.C0, RootCF * cf(0, 0, 0 - 0.15 * math.cos(sine / 20)) * angles(math.rad(90), math.rad(90), math.rad(0)), 0.3)
										Torso.Neck.C0 = clerp(Torso.Neck.C0+Vector3.new(0,0.25,-.1), NeckCF * angles(math.rad(30), math.rad(-90), math.rad(-90)), 0.3)
