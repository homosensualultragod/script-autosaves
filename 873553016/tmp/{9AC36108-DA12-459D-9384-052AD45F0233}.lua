...
until Character
	...
	text6.Position = UDim2.new(05435, 055, 0, 0)
...
ParticleEmitter = {Create = function(Parent, Color1, Color2, LightEmission, Size, Texture, Transparency, ZOffset, Accel, Drag, LockedToPart, VelocityInheritance, EmissionDirection, Enabled, LifeTime, Rate, Rotation, RotSpeed, Speed, VelocitySpread)
	...
	HandleWeld = CreateWeld(m, Character["Right Arm"], Handle, CFrame.new(0, 0, 0, 1, 0, 0, 0, 1, 0, 0, 0, 1), CFrame.new(-1.09519327, 0.737398148, 0.20399169922, 0, -1, 0, 0, 0, -1, 1, 0, 0))
		...
		for i = 0, 1, 0.1 do
			...
			game:GetService("Debris"):AddItem(bodyVelocity, 1)
								...
								elseif Type == "Snare" then
										...
										elseif Type == "Lit" then
													for i = 0, 1, 0.025 do
														swait(30)
														BlockEffect()(BrickColor.new("Bright red"), hit.Parent.Torso.CFrame, 1, 1, 1, 3, 3, 3, 0.07)
...
Break = {Create = function(brickcolor, cframe, x1, y1, z1)
	...
	local prt = CreatePart(EffectModel, "Glass", 0, 0, brickcolor, "Effect", Vector3.new(0.5, 0.5, 0.5))
	prt.Anchored = true
	prt.CFrame = cframe
	prt.Material = "Glass"
	...
	coroutine.resume(coroutine.create(function()
		...
		repeat
			swait()
			num2 = num2 + 0.1
			SpreadAmount = SpreadAmount * 1.05
			local hit, pos = rayCast(MainPos, MouseLook.lookVector, speed, RootPart.Parent)
			local mag = (MainPos - pos).magnitude
			Laser(BrickColor.new(color1), CFrame.new((MainPos + pos) / 2, pos) * angles(1.57, 0, 0), 1.5, mag * (speed / (speed / 2)), 1.5, -0.35, 0, -0.35, 0.05)
			...
			if hit ~= nil then
				...
				MagniDamage(refpart, 2, 6,6, -1, "Normal")
			...
			do
				if num <= 0 then
					...
					game:GetService("Debris"):AddItem(refpart2, 1)
						...
						Effects.Block.Create(color1, refpart2.CFrame, 1, 1, 1, 1,1,1, 0.05)
						MagniDamage(refpart2, 2, 6,6, -1, "Normal")
						--shoottraildd2(mouse, refpart2, (0))
					end
				end
		until num <= 0
	end
...
shoottraildd2 = function(mouse, partt, SpreadAmount)
	...
	coroutine.resume(coroutine.create(function()
		...
		repeat
			...
			local hit, pos = rayCast(MainPos, MouseLook.lookVector, speed, RootPart.Parent)
			local mag = (MainPos - pos).magnitude
			Laser(BrickColor.new(color1), CFrame.new((MainPos + pos) / 2, pos) * angles(1.57, 0, 0), 1, mag * (speed / (speed / 2)), 1, 0.25, 2, 0.25, 0.01)
			MainPos = MainPos + MouseLook.lookVector * speed
			num = num - 1
...
shoottraildd3 = function(mouse, partt, SpreadAmount)
	...
	while shoot do
		while shoot do
			if shoot == true and selectfire == 1 then
				swait(2.5/(1+overheat/5))
				if cooldown1 >= 1 then
					if overheat < 13 and oplevel == 1 then
					overheat = overheat + 0.12222
					end
					soe:Play()
					cooldown1 = cooldown1 - 0
				...
				if attack == false and aiming == false and hot == false and k == "x3213213" --[[and co2 <= cooldown2]] then
					...
					if k == "b" then
						...
						if attack == true and aiming == true and k == "q" then
							...
							if k == "vgfdg" and selectfire == 4 then
								selectfire = 1
							else
								if k == "vgdfgdfg" and selectfire ~= 4 then
	...
	while true do
		...
		if aiming == true then
			if 2 >= Torsovelocity then
			RootJoint.C0 = clerp(RootJoint.C0, RootCF * cf(0, 0, 0 - 0.15 * math.cos(sine / 20)) * angles(math.rad(90), math.rad(90), math.rad(0)), 0.3)
			Torso.Neck.C0 = clerp(Torso.Neck.C0+Vector3.new(0,0.25,-.1), NeckCF * angles(math.rad(30), math.rad(-90), math.rad(-90)), 0.3)
			RW.C0 = clerp(RW.C0, CFrame.new(1, 0.5, -0.5) * angles(math.rad(90), math.rad(0), math.rad(-90)), 0.3)
			LW.C0 = clerp(LW.C0, CFrame.new(-1.5, 0.5 - 0.1 * math.cos(sine / 25), 0) * angles(math.rad(90), math.rad(0), math.rad(-50)), 0.3)
			...
			if 2 < Torsovelocity then
										RootJoint.C0 = clerp(RootJoint.C0, RootCF * cf(0, 0, 0 - 0.15 * math.cos(sine / 20)) * angles(math.rad(90), math.rad(90), math.rad(0)), 0.3)
										Torso.Neck.C0 = clerp(Torso.Neck.C0+Vector3.new(0,0.25,-.1), NeckCF * angles(math.rad(30), math.rad(-90), math.rad(-90)), 0.3)
										RW.C0 = clerp(RW.C0, CFrame.new(1, 0.5, -0.5) * angles(math.rad(90), math.rad(10), math.rad(-90)), 0.3)
										LW.C0 = clerp(LW.C0, CFrame.new(-1.5, 0.5 - 0.1 * math.cos(sine / 25), 0) * angles(math.rad(90), math.rad(45), math.rad(-50)), 0.3)
										RH.C0 = clerp(RH.C0, CFrame.new(0.5, -2, 0 + 1 * math.cos((sine) / 3)) * angles(math.rad(0 - 50 * math.cos((sine) / 3)), math.rad(0), math.rad(0)), 0.3)
										LH.C0 = clerp(LH.C0, CFrame.new(-0.5, -2, 0 - 1 * math.cos((sine) / 3)) * angles(math.rad(0 + 50 * math.cos((sine) / 3)), math.rad(0), math.rad(0)), 0.3)
										FakeHandleWeld.C0 = clerp(FakeHandleWeld.C0, CFrame.new(0, 0, 0) * angles(math.rad(-40), math.rad(0), math.rad(-130)), 0.3)
										FakeMotorWeld.C0 = clerp(FakeMotorWeld.C0, CFrame.new(0, 0, 0) * angles(math.rad(0), math.rad(0), math.rad(0)), 0.3)
