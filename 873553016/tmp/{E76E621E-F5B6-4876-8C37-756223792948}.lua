...
Break = {Create = function(brickcolor, cframe, x1, y1, z1)
	...
	coroutine.resume(coroutine.create(function()
		...
		repeat
			...
			Laser(BrickColor.new(color1), CFrame.new((MainPos + pos) / 2, pos) * angles(math.rad(num*15), math.rad(0), math.rad(0)), 1, mag * (speed / (speed / 2)), 1, .15, 0, .15, 0.15)
			MainPos = MainPos + MouseLook.lookVector * speed
			MouseLook = MouseLook * angles(0, math.rad(0), 0)
			num = num - 1
			if hit ~= nil then
				num = 0
			...
			do
				if num <= 0 then
					...
					MagniDamage(refpart2, 5, 15 , 15, 1, "Normal")
				end
			end
			end
		until num <= 0
...
shoottraildd2 = function(mouse, partt, SpreadAmount)
	...
	coroutine.resume(coroutine.create(function()
		...
		repeat
			...
			local hit, pos = rayCast(MainPos, MouseLook.lookVector, speed, RootPart.Parent)
			local mag = (MainPos - pos).magnitude
			Laser(BrickColor.new(color1), CFrame.new((MainPos + pos) / 2, pos) * angles(math.rad(90), math.rad(-num*5), math.rad(90)), 1, mag * (speed / (speed / 2)), 1, .5, 0, .5, 0.15)
			MainPos = MainPos + MouseLook.lookVector * speed
			num = num - 1
			MouseLook = MouseLook * angles(math.rad(0), math.rad(0), 0)
							...
							if Torsovelocity < 1 and hit ~= nil then
								...
								if attack == false then
									change = 1
											...
											RW.C0 = clerp(RW.C0, CFrame.new(1, 0.5, -0.5) * angles(math.rad(90), math.rad(0), math.rad(-90)), 0.3)
											LW.C0 = clerp(LW.C0, CFrame.new(-1.5, 0.5 - 0.1 * math.cos(sine / 25), 0) * angles(math.rad(90), math.rad(0), math.rad(-70)), 0.3)
											RH.C0 = clerp(RH.C0, cf(.5, -2, 0) * RHCF * angles(math.rad(0), math.rad(-20), math.rad(0)), 0.3)
											LH.C0 = clerp(LH.C0, cf(-.5, -2, 0) * LHCF * angles(math.rad(0), math.rad(110), math.rad(0)), 0.3)
