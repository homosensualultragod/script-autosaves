	...
	for _,c in pairs(workspace:children()) do
		...
		if hum ~= nil then
			...
			if head ~= nil then
				...
				if Type == "Void" then
					if mag >= magni and mag < magni+10 then
						BlockEffect(BrickColor.new("Royal purple"), hum.Parent.Torso.CFrame*CFrame.new(0,4,0), 5, 5, 5, 3, 3, 3, 0.05, 1)
						local asdd = math.random(1,3)
						if asdd == 1 then
						shoottraildd(mouse, hum.Parent.Torso, 0)
...
Break = {Create = function(brickcolor, cframe, x1, y1, z1)
	...
	coroutine.resume(coroutine.create(function()
		...
		repeat
			...
			Laser(BrickColor.new(color1), CFrame.new((MainPos + pos) / 2, pos) * angles(1.57, 0, 0), 1, mag * (speed / (speed / 2)), -0.5, 0, -0.5, 1, 0.55)
			...
			do
				if num <= 0 then
					...
					if hit ~= nil then
						for i = 0,59,1 do
						Effects.Sphere.Create("Royal purple", refpart2.CFrame, 1, 1, 1, 5,5,5, 0.05)
						MagniDamage(refpart2, 2, 1,1, -1, "Void")
						swait(2.5)
						end
						void = false
					end
	...
	shoottrailddfake = function(mouse, partt, SpreadAmount)
	
	local SpreadVectors = Vector3.new(math.random(-SpreadAmount, SpreadAmount), math.random(-SpreadAmount, SpreadAmount), math.random(-SpreadAmount, SpreadAmount))
	local MainPos = partt.Position+Vector3.new(0,4,0)
	local MainPos2 = mouse.Hit.p + SpreadVectors
	local MouseLook = CFrame.new((MainPos + MainPos2) / 2, MainPos2)
	local speed = 150
	local num = 1
	...
	coroutine.resume(coroutine.create(function()
		...
		repeat
			...
			local hit, pos = rayCast(MainPos, MouseLook.lookVector, speed, RootPart.Parent)
			local mag = (MainPos - pos).magnitude
			Laser(BrickColor.new("Royal purple"), CFrame.new((MainPos + pos) / 2, pos) * angles(1.57, 0, 0), 1, mag * (speed / (speed / 2)), -0.25, 0, -0.25, 1, 0.15)
			MainPos = MainPos + MouseLook.lookVector * speed
			num = num - 1
			...
			do
				if num <= 0 then
					...
					refpart2.Anchored = true
					refpart2.CFrame = CFrame.new(pos)
					MagniDamage(refpart2, 5, 1  * dermage , 1  * dermage, 0, "Normal")
					game:GetService("Debris"):AddItem(refpart2, 1)
					if hit ~= nil then
					end
				end
