		...
		if hum ~= nil then
			...
			if head ~= nil then
				...
				if mag <= magni and c.Name ~= Player.Name then
					Effects.Block.Create(color1, head.CFrame, 1,1,1,10,10,10,0.05)
					Effects.Block.Create(color2, head.CFrame, 1, 1, 1, 25,2.5,2.5, 0.05)
...
Break = {Create = function(brickcolor, cframe, x1, y1, z1)
	...
	coroutine.resume(coroutine.create(function()
		...
		repeat
			...
			num = num - 1
			num2 = num2 + 1
			if num2 > 5 and up == false then
			num2 = 0
			started = true
			up = true
			end
			if num2 > 5 and up == true then
			num2 = 0
			started = true
			up = false
			end
			if up == true and started == true then
			MouseLook = MouseLook * angles(0, 0.2, 0)
			end
			if up == false and started == true then
			MouseLook = MouseLook * angles(0, -0.2, 0)
			end
			if hit ~= nil then
				num = 0
		...
		while shoot do
			if shoot == true and selectfire == 1 then
				...
				if cooldown1 >= 1 then
					...
					shoottraildd2(mouse, Barrel, (0))
					shoottraildd3(mouse, Barrel, (0))
				else
					soe:Stop()
					reload()
					aiming = false
