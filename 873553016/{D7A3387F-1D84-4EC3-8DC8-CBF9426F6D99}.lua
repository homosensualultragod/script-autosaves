...
Break = {Create = function(brickcolor, cframe, x1, y1, z1)
	...
	coroutine.resume(coroutine.create(function()
		...
		repeat
			...
			BlockEffect(BrickColor.new(color1), CFrame.new((MainPos + pos) / 2, pos) * angles(1.57, 0, 0), 1, mag * (speed / (speed / 2)), 1, 15,15,15, 0.15)
			...
			MagniDamage(refpart2, 5, 0.5  * dermage , 0.5  * dermage, 1, "Normal")
			if hit ~= nil then
				speed = 0.01
			end
		until num <= 0
...
shoottraildd2 = function(mouse, partt, SpreadAmount)
	...
	coroutine.resume(coroutine.create(function()
		...
		repeat
			...
			local hit, pos = rayCast(MainPos, MouseLook.lookVector, speed, RootPart.Parent)
			local mag = (MainPos - pos).magnitude
			BlockEffect(BrickColor.new(color1), CFrame.new((MainPos + pos) / 2, pos) * angles(1.57, 0, 0), 1, mag * (speed / (speed / 2)), 1, 25,25,25, 0.15)
			MainPos = MainPos + MouseLook.lookVector * speed
			num = num - 1
			MouseLook = MouseLook * angles(math.rad(-2), 0, 0)
			local refpart2 = CreatePart(EffectModel, "SmoothPlastic", 0, 1, BrickColor.new("Really black"), "Effect", Vector3.new())
			refpart2.Anchored = true
			refpart2.CFrame = CFrame.new(pos)
			game:GetService("Debris"):AddItem(refpart2, 1)
			MagniDamage(refpart2, 15, 0.5  * dermage , 0.5  * dermage, 1, "Normal")
			if hit ~= nil then
				speed = 0.01
			end
		until num <= 0
		...
		while shoot do
			if shoot == true and selectfire == 1 then
				...
				if cooldown1 >= 1 then
					...
					if oplevel == 1 then
					shoottraildd(mouse, Barrel, (1+overheat/4))
					end
					if oplevel == 2 then
					shoottraildd2(mouse, Barrel, (1+overheat/4))
					swait(2.5)
					shoottraildd2(mouse, Barrel, (1+overheat/4))
					swait(2.5)
					shoottraildd2(mouse, Barrel, (1+overheat/4))
